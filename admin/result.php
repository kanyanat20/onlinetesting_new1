...
$id = $_GET['id'];
$sql_subject = "SELECT * FROM subject_information WHERE subject_id = '$id'";
$result_subject = $conn->prepare($sql_subject);
$result_subject->execute();
$row_subject = $result_subject->fetch(PDO::FETCH_ASSOC);
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">ผลสอบวิชา <?php echo $row_subject['subject_name']; ?></h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="index.php">หน้าหลัก</a></li>
              
              <li class="breadcrumb-item active">ผลสอบ</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
        <div>
            <!-- /.Table to excel -->
            <a href="export.php?id=<?php echo $id; ?>" class="btn btn-success">Export to Excel</a>
        </div>
      </div><!-- /.container-fluid -->
    </div>
    <!-- /.content-header -->
    ...
    while($row = $result->fetch(PDO::FETCH_ASSOC)){
        // ตรวจสอบว่า urole เป็น 'admin' หรือไม่
        if ($row['urole'] !== 'admin') {
